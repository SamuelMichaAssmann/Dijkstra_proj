...
public class DijkstraGraph<T> {
    ...
    public Path<T> compute(T sourcePoint, T destinationPoint) {
        if ( this.nodes.stream().noneMatch( node -> node.getPoint().equals(sourcePoint) ) )   {
            throw new IllegalArgumentException("The desired source point is not defined in the " +
                    "provided set of nodes!");
        }
        if ( this.nodes.stream().noneMatch( node -> node.getPoint().equals(destinationPoint) ) )   {
            throw new IllegalArgumentException("The desired destination point is not defined in " +
        ...
        while( unvisitedPoints.contains(destinationPoint) )   {
            ...
            if ( unvisitedPoints.contains(destinationPoint) )    {
                ...
                final T newCurrentPoint
                        = findUnvisitedPointWithLowestAssumedCost(costs, unvisitedPoints);
                // ^ because the java-Lambda only accepts final vars

                currentNode = nodes.stream()
                        .filter( node -> node.getPoint().equals(newCurrentPoint) )
                        .findFirst().get();
        ...
        return new Path<>(sourcePoint, destinationPoint,
                currentCost, reconstructBestPath(sourcePoint, destinationPoint ,predecessors));
    }
     ...
     */
    private void updatePeersForLowerCost(Map<T, Double> costs, Map<T, T> predecessors,
                                         Node<T> currentNode, Double currentCost)
    {
    ...
    private List<T> reconstructBestPath          /* (a.k.a the one with the most northest points) */
    (T sourcePoint, T destinationPoint, Map<T, T> predecessorMap)
    {
